...
const ProjectPreview = forwardRef<ProjectPreviewRef, ProjectPreviewProps>(({ project, isGenerating, device = 'desktop', showEditorPanel = true }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
        getCode: () => {
            ...
            // 1. Remove our  selection class / attributes / outline from all elemnts 
            doc.querySelectorAll('.ai-selected-elementt , [data-ai-selected]').forEach((el) => {
                el.classList.remove('ai-selected-element');
                el.removeAttribute('data-ai-selected');
                (el as HTMLElement).style.outline = '';
            })

            // 2. Remove injected stle + script from the document 
            ...
            {project.current_code ? (
                ...
                <>
                    ...
                    {showEditorPanel && selectedElement && (
                        <EditorPannel selectedElement={selectedElement} onUpdate={handleUpdate} onClose={() => {
                            setSelectedElement(null);
                            if (iframeRef.current?.contentWindow) {
